...
using namespace UInjectLoader;

std::string UInjectLoader::ToString(std::wstring wstr) {
...
System::String^ UInjectLoader::ToSysString(std::string str) {
	System::String^ str2 = gcnew System::String(str.c_str());
	return str2;
}

std::string UInjectLoader::ToString(System::String^ str) {
	msclr::interop::marshal_context context;
	return context.marshal_as<std::string>(str);
}

std::wstring UInjectLoader::ToWString(std::string str) {
	using ccvt = std::codecvt_utf8<wchar_t>;
	std::wstring_convert<ccvt, wchar_t> converter;
...
std::string UInjectLoader::CurrentPath()
{
	...
	if (bytes > 0)
	{
		...
		ret = ret.substr(0, ret.size() - 16); // UGLY - HACK
...
void* UInjectLoader::GetProcAddressEx(void* pModuleBase, std::string procName)
{
	...
	if (pNtHdrs32->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC)
	{
		...
		for (ULONG i = 0; i < numberOfNames; i++) {
			...
			if (std::string(pszName).find(procName) != std::string::npos) {
...
std::vector<char> UInjectLoader::FileReadAllBytes(const std::wstring& name) {
